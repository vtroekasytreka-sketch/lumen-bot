...
- `claude_client.py` — обёртка над OpenRouter (`call_claude`, `call_claude_chat`)
- ~~`groq_client.py` / `grok_client.py` / `dialog_manager.py`~~ — устарели после перехода на Claude, не импортируются новым `bot.py`
...
| `OPENROUTER_API_KEY` | API ключ OpenRouter (модель `anthropic/claude-sonnet-4-5`) |
